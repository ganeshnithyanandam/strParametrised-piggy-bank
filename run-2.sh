#!/bin/bash
export MOM=$(curl -s -H "Content-Type: application/json" -X POST -d '{"caID": "StrParameterisedPiggyBankContract", "caWallet":{"getWallet": 1}}' http://localhost:9080/api/contract/activate | jq .unContractInstanceId | tr -d '"')
export DAD=$(curl -s -H "Content-Type: application/json" -X POST -d '{"caID": "StrParameterisedPiggyBankContract", "caWallet":{"getWallet": 2}}' http://localhost:9080/api/contract/activate | jq .unContractInstanceId | tr -d '"')
